package com.carlv.medicaldeliverydoctor.Activities.Delivery;
...
import com.carlv.medicaldeliverydoctor.Helpers.ChartStyler;
...
import com.carlv.medicaldeliverydoctor.Menu.BottomSlideMenu;
...
public class DeliveryDetailActivity extends AppCompatActivity implements OnMapReadyCallback, GoogleMap.OnMarkerClickListener {
    ...
    private void setOrder(int status) {
        Service service = restClient.getClient().create(Service.class);
        delivery.setStatus(status);
        delivery.setResponse(true);
        Call<Delivery> jsonCall = service.updateDelivery(delivery.getID().toString(), delivery);
        jsonCall.enqueue(new Callback<Delivery>() {
            @Override
            public void onResponse(Call<Delivery> call, Response<Delivery> response) {
                Toast.makeText(DeliveryDetailActivity.this, "Updated", Toast.LENGTH_SHORT).show();
                loadDelivery();
            }
    ...
    private void dialogCreater() {
        ...
        Button btnOkay = dialog.findViewById(R.id.dia_issue_btnOkay);
        Button btnCancel = dialog.findViewById(R.id.dia_issue_btnCancel);
        Button btnReorder = dialog.findViewById(R.id.dia_issue_btnReorder);
        ...
        btnReorder.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                setOrder(4);
                postDelivery(delivery.getMedicine().getID(), STATIC.hospitalID, dialog);
            }
        });
        dialog.show();
    }
    ...
    private void postDelivery(int medicineID, int hospitalID, final Dialog dialog) {
            ...
            public void onResponse(Call<Delivery> call, Response<Delivery> response) {
                ...
                if (delivery != null) {
                    ...
                    if (responseCode != null && responseCode.equals("404")) {
                        Toast.makeText(DeliveryDetailActivity.this, "Error: Not Found", Toast.LENGTH_SHORT).show();
                    } else {
                        Log.i("Success", "Success");
                        Toast.makeText(DeliveryDetailActivity.this, "Reordered " + delivery.getMedicine(), Toast.LENGTH_SHORT).show();
                        dialog.dismiss();
                    }
                }
            }
